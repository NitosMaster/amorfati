...
## math...
I didn't wanna use cmath or openglmathematics and so I wrote my own vectors, matrices and quaternions. By doing this I learned a lot more about linear algebra and fulfilled my obsession with useless optimizations the compiler would fix automatically, however it was a little bit of a pain in the ass. Seeing when I lose my mind in commits or comments is a good drinking game. If you went into src/amorfati/math and didn't understand one shit, dw it took me sometime to understand SIMD instructions either. Basically the CPU has funcs like sqrt() inside it. And when writing sqrt() the func gives the compiler all the work (as can be seen by going into the GNU cmath src). The compiler then may decide wheter to use the Newton-Raphson method or just use the hardware sqrt(), which from my knowledge is also a Newton-Raphson method but built into hardware. Essentially I force my program to use hardware as it's faster. Sadly using SIMD and malloc is pretty annoying as openGL desires a specific format for most of it's funcs. As indicated by my comment in Matrix.cpp, I did use cmath. This is because the hardware sin/cos/tan is supposedly slower than cmath and I didn't want to do a local optimization as I am not well educated on computing methods of trig funcs. I have no mouth and I must scream (also seeing the triangle rotate and change color was so satisfying, it's crazy). 
